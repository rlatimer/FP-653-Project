...
```{r}
#Racheals cdc plots
cdc_plots <- app_data %>% #need to reference county_covid, watch out for some column names for the time being 
  group_by( fips) %>% 
  nest() %>% 
  mutate(plot = pmap(list(fips, data), ~{
    ggplot(..2, aes(x=month,
      ...
      scale_x_discrete(limits = factor(c(0,max(app_data$month))), 
                       expand = c(0, 0)) +
